...
class TacticalViewConverter:
    def __init__(self, court_image_path, court_config=None):
        self.court_image_path = court_image_path
        self.court_config = court_config or BasketballCourtConfiguration()

        # Tactical coordinate system now follows the model configuration exactly.
        self.width = int(self.court_config.length)
        self.height = int(self.court_config.width)

        self.key_points = [
            (float(x), float(y))
            for x, y in self.court_config.vertices
        ]
